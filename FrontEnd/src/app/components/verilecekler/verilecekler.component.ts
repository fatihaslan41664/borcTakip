...
import { VerilecekService } from '../../services/verilecek.service';
import { ToastrService } from 'ngx-toastr';

@Component({
  selector: 'app-verilecekler',
  imports: [CommonModule, FormsModule],
  templateUrl: './verilecekler.component.html',
  styleUrl: './verilecekler.component.css'
})
export class VerileceklerComponent implements OnInit {
  liste: Alinacak[] = [];
  ...
  constructor(
    private verilecekService: VerilecekService,
    private toastr: ToastrService
  ) {}

  ngOnInit() {
    this.verilecekService.GetAll().subscribe((data: Alinacak[]) => {
  ...
  kaydet() {
    ...
    this.verilecekService.Ekle(this.yeniKayit.isim, this.yeniKayit.borcMiktari, tarih).subscribe(() => {
      this.toastr.success('Kayıt başarıyla eklendi.', 'Başarılı');
      this.modalKapat(); this.ngOnInit();
    });
  }
  ...
  silOnayla() {
    this.verilecekService.Sil(this.silinecekId).subscribe(() => {
  ...
  borcOde(hepsiniOde: boolean = false) {
    ...
    this.verilecekService.BorcOde(this.seciliKayit.id, this.odenenMiktar, hepsiniOde).subscribe(() => {
      this.toastr.success('Ödeme başarıyla kaydedildi.', 'Başarılı');
      this.borcOdeKapat(); this.ngOnInit();
    });
  }
